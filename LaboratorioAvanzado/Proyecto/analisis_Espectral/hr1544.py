...
espectre = np.loadtxt("HR1544Prom.dat")
star = 'hr1544'
...
beta = 2*A/np.pi/np.max(linea) *(3*np.pi+8)
c = 299792458
...
yBaja = np.array([3.832,7.016,10.174])
...
vsini2 = heh.mean()*c*1e-5/1000 , np.sqrt(cvr.mean())*c*1e-5/1000
#plt.plot(x,heh[0]*x)
heh = y/(x)
